...
def get_client_by_cid(db: Session, client_id: int):
    return db.query(client_credentials.ClientCredentials).filter(client_credentials.ClientCredentials.client_id == client_id).first()

def create_client(db: Session, client: client_schema.Client):
    db_client = client_credentials.ClientCredentials(client_id=client.client_id, client_secret=security.get_password_hash(client.client_secret), role=client.role)
    db.add(db_client)
    db.commit()
